...
export class HoldingRoom extends Room<RoomState> {
  ...
  private startVirusBattle(): void {
    ...
    const GRID_TOTAL = HoldingRoom.GRID_TOTAL; // 800 cells

    // Initialize the virus battle state
    // Set up the vGrid (40x20 = 800 cells)
    ...
    if (players.length >= 2) {
      ...
      // Initialize vGrid with starting positions (4 corners)
      // Virus A (RED) - Player 1 (room creator) - TOP-LEFT
      const topLeftX = 2;
      const topLeftY = 2;
      ...
      // Virus B (BLUE) - Player 2 (joiner) - TOP-RIGHT
      const topRightX = GRID_WIDTH - 3;
      const topRightY = 2;
      const topRightIndex = topRightY * GRID_WIDTH + topRightX;
      if (topRightIndex < this.state.vGrid.length) {
        this.state.vGrid[topRightIndex] = 2; // 2 = VIRUS_B (BLUE)
      }

      // Virus C (YELLOW) - BOTTOM-LEFT
      const bottomLeftX = 2;
      const bottomLeftY = GRID_HEIGHT - 3;
      const bottomLeftIndex = bottomLeftY * GRID_WIDTH + bottomLeftX;
      if (bottomLeftIndex < this.state.vGrid.length) {
        this.state.vGrid[bottomLeftIndex] = 1; // 1 = VIRUS_A (RED)
      }

      // Virus D (PURPLE) - BOTTOM-RIGHT
  ...
  private startBattleSimulation(): void {
    ...
    console.log('[BATTLE] Starting battle simulation with 500ms ticks');
    ...
    }, 500); // 500ms per tick (2 ticks/second) - matches client BattleManager

    // Store interval reference to allow cleanup
    (this as any).battleInterval = battleInterval;
  }

  // Grid dimensions for multiplayer battles (must match client expectations)
  private static readonly GRID_WIDTH = 40;
  private static readonly GRID_HEIGHT = 20;
  private static readonly GRID_TOTAL = HoldingRoom.GRID_WIDTH * HoldingRoom.GRID_HEIGHT;

  private updateVirusSpread(): void {
    console.log('[BATTLE] Running virus spread tick');
        ...
        if (cell !== 0) {
          ...
          for (const neighbor of neighbors) {
            if (neighbor.x >= 0 && neighbor.x < GRID_WIDTH && neighbor.y >= 0 && neighbor.y < GRID_HEIGHT) {
              const nIdx = neighbor.y * GRID_WIDTH + neighbor.x;
              if (this.state.vGrid[nIdx] === 0) { // Empty cell
              ...
              } else if (this.state.vGrid[nIdx] !== cell) { // Opposing virus
    ...
    console.log(`[BATTLE] Spread count: ${spreadCount}, Grid state sent to clients`);

    // Broadcast the updated grid to all players (include dimensions for dynamic renderer)
  ...
  private checkWinConditions(): boolean {
    // Check if one virus has taken over 96% of the grid (was 99%)
    ...
    // Win condition: one virus controls 96% or more of the grid
    if (virusAPercent >= 96) {
